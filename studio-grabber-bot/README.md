...
Telegram-бот — **копилка крутых референсов студии**: увидел классный ролик или
пост → кинул ссылку в бот → он скачивает в **максимальном качестве**
(`yt-dlp` + `gallery-dl`), складывает в общую папку **Google Drive** и присылает
ссылку.

Стратегия: **всё через облако + ссылка** — нет ограничения Telegram в 50 МБ,
тяжёлое 4K-видео отдаётся без проблем.
...
- Всё складывается в одну общую папку-копилку референсов (без сортировки).
  При желании можно включить раскладку по дате флагом `DATE_SUBFOLDERS=true`.
